...
describe('timer', function () {
  ...
  describe('update()', () => {
    it('returns a new timer with updated state', () => {
      const updated = timer.update(result, {
        minutes: 4
      });
      expect(updated.state.minutes).toBe(4);
    });

    it('should validate the new state', () => {
      const updated = timer.update(result, {
        minutes: 4,
        seconds: 61
      });

      expect(updated.state).toEqual({minutes: 5, seconds: 1});
    });

    it('should emit an update event', () => {
      let updated;
      result.on('update', u => updated = u);
      timer.update(result, {minutes: 4});
      expect(updated.state).toEqual({minutes: 4, seconds: 0});
    });
  });
