...
describe('timer', function () {
  ...
  describe('diff()', () => {
    ...
    it('should return a new timer object with a timestamp containing the remaining time', () => {
      ...
      expect(newTimer.state.minutes).toBe(2);
      expect(diff).toBeCloseTo(60);
    });
    ...
    it('should emit a diff event on a shared emitter', () => {
      ...
      expect(diffed.state.minutes).toBe(2);
      expect(diff).toBeCloseTo(60);
    });
  ...
  describe('start()', () => {
    ...
    it('should register a listener to stop when the diff reaches 0', () => {
      let stopped;
      const threeMinutesFromNow = Date.now() + 1000 * 60 * 3;
      const newTimer = timer.start(result);
      newTimer.on('stop', t => stopped = t);

      timer.diff(newTimer, threeMinutesFromNow);
